...
async function main() {
  ...
  ctx.p("El Instituto Universitario de Ciencias de la Educación (IUCE) de la Universidad de Salamanca disponía de una web institucional construida sobre WordPress y alimentada desde 2010, que había quedado desactualizada en lo tecnológico y en lo funcional: un gestor genérico, contenido en un único idioma y ninguna integración con las fuentes de datos actuales del Instituto (portal de producción científica, memoria de acreditación, sistema de reservas de espacios).");
  ...
  ctx.bullets([
    [t("La aplicación web completa, con su código fuente bajo control de versiones (Git, rama "), c("main"), t("), lista para desplegar en la infraestructura institucional.")],
  ...
  ctx.bullets([
    ...
    [b("CMS headless (Strapi, Directus…) con frontend aparte. "), t("Separa el gestor del sitio, pero duplica infraestructura y lenguajes de configuración, y el gestor genérico habría exigido tanta personalización como el panel propio para lograr la edición por bloques y listas que se buscaba. El panel a medida, construido sobre el mismo patrón que ya usa el gestor de la web del MUPES, ofrece exactamente los formularios que el personal necesita, en castellano y sin conceptos ajenos.")],
  ...
  ctx.p("El desarrollo partió de una fase de diseño con prototipos navegables de todas las páginas (conservados en docs/design junto con el sistema de diseño y la especificación de handoff), que fijó la estructura y la identidad visual antes de construir. La implementación avanzó por áreas funcionales completas —páginas públicas, migración del histórico, panel, editabilidad total, versión inglesa, área de miembros, estadísticas—, incorporando de forma continua las indicaciones de la dirección y del personal del Instituto (jerarquía de contenidos, responsables de grupo, textos institucionales, visibilidad de secciones).");
  ...
  ctx.bullets([
    ...
    [t("IUCE Reservas — sistema de gestión de reservas de espacios del IUCE (Trabajo de Fin de Grado en Ingeniería Informática, Universidad de Burgos, 2026): "), c("https://reservas.iuce.usal.es")],
  ]);
